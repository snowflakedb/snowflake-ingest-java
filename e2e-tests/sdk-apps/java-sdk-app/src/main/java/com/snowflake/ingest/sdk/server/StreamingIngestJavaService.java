...
public class StreamingIngestJavaService {
  ...
  private static final Map<String, Map<String, SnowflakeStreamingIngestChannel>> channelsMap =
      new ConcurrentHashMap<>();
  ...
  public HttpResponse createClient(
      @Param String clientId, @RequestObject CreateClientRequest request) {
    try {
      ...
      try {
        ...
        SnowflakeStreamingIngestClientFactory.Builder builder =
            SnowflakeStreamingIngestClientFactory.builder(clientId)
                .setProperties(props)
                .setIsTestMode(
                    Boolean.parseBoolean(
                        overrides.getOrDefault("ROWSET_DEV_VM_TEST_MODE", "false")));
        ...
        if (client == null) {
          throw new AppServerException(
              AppServerException.AppErrorCode.UNEXPECTED_ERROR, "Client builder returned null");
        }
        ...
        throw new AppServerException(
            AppServerException.AppErrorCode.UNEXPECTED_ERROR, "Failed to build client: " + msg);
      }
      ...
      throw new AppServerException(
          AppServerException.AppErrorCode.UNEXPECTED_ERROR,
  ...
  public HttpResponse getLatestCommittedOffsetToken(
      @Param String clientId, @Param String channelName) {
    try {
      SnowflakeStreamingIngestChannel channel = getChannelOrThrow(clientId, channelName);
  ...
  public HttpResponse closeChannel(@Param String clientId, @Param String channelName) {
    try {
      SnowflakeStreamingIngestChannel channel = getChannelOrThrow(clientId, channelName);
      channel.close().get();
  ...
  private SnowflakeStreamingIngestClient getClientOrThrow(String clientId) {
    ...
    if (client == null) {
      throw new AppServerException(
          AppServerException.AppErrorCode.INVALID_INPUT, "Client not found: " + clientId);
    }
    return client;
  }
  ...
  private SnowflakeStreamingIngestChannel getChannelOrThrow(String clientId, String channelName) {
    ...
    if (channel == null) {
      throw new AppServerException(
          AppServerException.AppErrorCode.INVALID_INPUT,

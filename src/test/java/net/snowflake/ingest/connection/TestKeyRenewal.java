 ...
 * is successfully resetting the the JWT Token after the specified timespan
 */
public class TestKeyRenewal
{
      ...
      throws NoSuchProviderException, NoSuchAlgorithmException, InterruptedException
  {
    ...
    SecurityManager manager = new SecurityManager("account", "user", keyPair, 3, TimeUnit.SECONDS);

    //grab a token
    String token = manager.getToken();

    //if we immediately request the same token, it should be exactly the same
    assertEquals("Two token requests prior to renewals should be the same", token, manager.getToken());

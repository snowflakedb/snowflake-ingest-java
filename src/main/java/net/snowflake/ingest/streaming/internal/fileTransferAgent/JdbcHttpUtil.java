 ...
 * Source: https://github.com/snowflakedb/snowflake-jdbc/blob/v3.25.1/src/main/java/net/snowflake/client/core/HttpUtil.java
 *
 * Permitted differences: package declaration, class renamed from HttpUtil to JdbcHttpUtil
 ...
 * SnowflakeMutableProxyRoutePlanner uses ingest version (same package),
 * SessionUtil.isNewRetryStrategyRequest inlined from JDBC SessionUtil.
 * SnowflakeUtil.isNullOrEmpty -> StorageClientUtil.isNullOrEmpty.
 * SnowflakeUtil.systemGetProperty -> StorageClientUtil.systemGetProperty.
 * net.snowflake.client.log.* -> ...fileTransferAgent.log.*
 * net.snowflake.client.util.SecretDetector -> ...log.SecretDetector
 * net.snowflake.client.util.Stopwatch -> net.snowflake.ingest.utils.Stopwatch
 */
...
public class JdbcHttpUtil {
  ...
  static String buildUserAgent(String customSuffix) {
    ...
    builder.append(SnowflakeDriverConstants.implementVersion);
    builder.append(" (");
  ...
  public static CloseableHttpClient buildHttpClient(
    ...
    logger.debug(
        "Building http client with client settings key: {}, ocsp cache file: {}, download"
            + " uncompressed: {}",
    ...
    try {
      ...
      Registry<ConnectionSocketFactory> registry =
          RegistryBuilder.<ConnectionSocketFactory>create()
              .register(
                  "https", new IngestSSLConnectionSocketFactory(trustManagers, socksProxyDisabled))
  ...
  private static void initDefaultRequestConfig(long connectTimeout, long socketTimeout) {
    ...
    logger.debug(
        "Rebuilding request config. Connect timeout: {} ms, connection request timeout: {} ms,"
            + " socket timeout: {} ms",
        connectTimeout,
        connectTimeout,
        socketTimeout);
  ...
  public static void setSocksProxyDisabled(boolean socksProxyDisabled) {
    ...
    JdbcHttpUtil.socksProxyDisabled = socksProxyDisabled;
  }
  ...
  public static boolean isSocksProxyDisabled() {
    return JdbcHttpUtil.socksProxyDisabled;
  }
  ...
  private static String executeRequestInternal(
    ...
    logger.debug(
        "Pool: {} Executing: {}",
        (ArgSupplier) JdbcHttpUtil::getHttpClientStats,
        requestInfoScrubbed);
    ...
    HttpExecutingContext context =
        HttpExecutingContextBuilder.forSimpleRequest(requestIdStr, requestInfoScrubbed)
            ...
            .unpackResponse(true)
            .noRetry(false)
            .loginRequest(isNewRetryStrategyRequest(httpRequest))
            .build();
    responseText =
  ...
  /**
   * Inlined from SystemUtil.convertSystemPropertyToIntValue. Source:
   * https://github.com/snowflakedb/snowflake-jdbc/blob/v3.25.1/src/main/java/net/snowflake/client/core/SystemUtil.java
   *
  ...
  /**
   * Inlined from SessionUtil.isNewRetryStrategyRequest. Source:
   * https://github.com/snowflakedb/snowflake-jdbc/blob/v3.25.1/src/main/java/net/snowflake/client/core/SessionUtil.java
   *
   * <p>Helper method to check if the request path is a login/auth request to use for retry
   * strategy.
   *
  ...
  public static boolean isNewRetryStrategyRequest(HttpRequestBase request) {
    // These constants are from SessionUtil in JDBC

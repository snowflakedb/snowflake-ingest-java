 ...
 * Source: https://github.com/snowflakedb/snowflake-jdbc/blob/v3.25.1/src/main/java/net/snowflake/client/jdbc/RestRequest.java
 *
...
import net.snowflake.ingest.streaming.internal.fileTransferAgent.log.Event;
import net.snowflake.ingest.streaming.internal.fileTransferAgent.log.EventUtil;
...
public class RestRequest {
   ...
   * @throws net.snowflake.ingest.streaming.internal.fileTransferAgent.SnowflakeSQLException Request
   *     timeout Exception or Illegal State Exception i.e. connection is already shutdown etc
   */
  ...
  public static HttpResponseContextDto executeWithRetries(
    ...
    if (logger.isDebugEnabled()) {
      ...
      logger.debug(
          "{}Executing rest request: {}, retry timeout: {}, socket timeout: {}, max retries: {},"
              + " inject socket timeout: {}, canceling: {}, without cookies: {}, include retry"
              + " parameters: {}, include request guid: {}, retry http 403: {}, no retry: {}",
    ...
    while (true) {
      logger.debug(
          "{}Retry count: {}, max retries: {}, retry timeout: {} s, backoff: {} ms. Attempting"
              + " request: {}",
          httpExecutingContext.getRequestId(),
  ...
  static Exception handlingNotRetryableException(
    ...
    if (ex instanceof IllegalStateException) {
      throw new SnowflakeSQLLoggedException(
          ErrorCode.INVALID_STATE, ex, /* session= */ ex.getMessage());
    } else if (isExceptionInGroup(ex, sslExceptions) && !isProtocolVersionError(ex)) {
      String formattedMsg =
          ex.getMessage()
              + "\n"
              + "Verify that the hostnames and portnumbers in SYSTEM$ALLOWLIST are added to your"
              + " firewall's allowed list.\n"
      ...
      throw new SnowflakeSQLLoggedException(ErrorCode.NETWORK_ERROR, ex, formattedMsg);
    ...
    if (!skipRetrying && isNonRetryableHTTPCode(response, httpExecutingContext.isRetryHTTP403())) {
      ...
      try {
        if (response == null || response.getStatusLine().getStatusCode() != 200) {
          ...
          logResponseDetails(response, logger);
  ...
  /**
   * Inlined from SnowflakeUtil.logResponseDetails — cannot call JDBC's version because it expects
   * net.snowflake.client.log.SFLogger, not our replicated SFLogger.
   */
  private static void logResponseDetails(HttpResponse response, SFLogger logger) {
    if (response == null) {

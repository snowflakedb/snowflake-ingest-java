...
class StreamingIngestExternalVolume extends UploadService {
   ...
   * @param stageMetadata external volume information received from open channel
          ...
          new SnowflakeFileTransferMetadataWithAge(
              (SnowflakeFileTransferMetadataV1)
                  SnowflakeFileTransferAgent.getFileTransferMetadatas(
                          parseStageMetadata(stageMetadata))
                      .get(0),
              Optional.of(System.currentTimeMillis()));
      this.volumeHash = stageMetadata.getVolumeHash();
    } else {
  ...
  StreamingIngestExternalVolume(
      ...
      String clientName,
      StageMetadata stageMetadata,
      UploadService.SnowflakeFileTransferMetadataWithAge testMetadata,
      int maxRetryCount)
      throws SnowflakeSQLException {
    this(isTestMode, role, httpClient, requestBuilder, clientName, stageMetadata, maxRetryCount);
    if (!isTestMode) {
  ...
  private JsonNode parseStageMetadata(StageMetadata stageMetadata) {
    Map<String, Object> response = new HashMap<>();
    response.put("stage_location", mapper.valueToTree(stageMetadata));

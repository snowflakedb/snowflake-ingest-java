...
class ParquetValueParser {
    ...
    static ParquetBufferValue parseColumnValueToParquet(
        ...
        if (value != null) {
            ...
            switch (typeName) {
                case BOOLEAN:
                    int intValue =
                            DataValidationUtil.validateAndParseBoolean(columnDisplayName, value);
                    value = intValue > 0;
                ...
                case DOUBLE:
                    double doubleValue =
                            DataValidationUtil.validateAndParseReal(columnDisplayName, value);
                    value = doubleValue;
    ...
    private static String getBinaryValue(
            ColumnDisplayName columnDisplayName, Object value, RowBufferStats stats, ColumnMetadata columnMetadata) {
        AbstractRowBuffer.ColumnLogicalType logicalType =
                AbstractRowBuffer.ColumnLogicalType.valueOf(columnMetadata.getLogicalType());
        String str;
        if (logicalType.isObject()) {
            switch (logicalType) {
                case OBJECT:
                    str = DataValidationUtil.validateAndParseObject(columnDisplayName, value);
                    break;
                case VARIANT:
                    str = DataValidationUtil.validateAndParseVariant(columnDisplayName, value);
                    break;
                case ARRAY:
                    str = DataValidationUtil.validateAndParseArray(columnDisplayName, value);
                    break;
                default:
                    throw new SFException(
            ...
            str =
                    DataValidationUtil.validateAndParseString(
                            columnDisplayName, value, Optional.of(maxLengthString).map(Integer::parseInt));
    ...
    private static byte[] getBinaryValueForLogicalBinary(
        ...
        byte[] bytes =
                DataValidationUtil.validateAndParseBinary(columnDisplayName, value, Optional.of(maxLengthString).map(Integer::parseInt));

...
public class Utils {
  ...
  public static Properties createProperties(Properties inputProp) {
    ...
    if (!properties.containsKey(Constants.AUTHORIZATION_TYPE)) {
      properties.put(Constants.AUTHORIZATION_TYPE, Constants.KEYPAIR_JWT);
    }

    String authType = properties.get(Constants.AUTHORIZATION_TYPE).toString();
    if (authType.equals(Constants.KEYPAIR_JWT)) {
      ...
      if (!Boolean.parseBoolean(
          properties.getOrDefault(Constants.OAUTH_AUTO_REFRESH, false).toString())) {
        if (!properties.containsKey(Constants.OAUTH_ACCESS_TOKEN)) {
          throw new SFException(ErrorCode.MISSING_CONFIG, Constants.OAUTH_ACCESS_TOKEN);
        }
      } else {
      ...
      throw new SFException(
          ...
          String.format(
              "authorization_type, should be %s or %s", Constants.KEYPAIR_JWT, Constants.OAUTH));
    }

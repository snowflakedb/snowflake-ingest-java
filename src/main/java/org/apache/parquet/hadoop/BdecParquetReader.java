...
public class BdecParquetReader implements AutoCloseable {
  ...
  public BdecParquetReader(byte[] data, BufferAllocator arrowAllocator) throws IOException {
    allocator = new ArrowByteBufferAllocator(arrowAllocator, false);
    ParquetReadOptions options = ParquetReadOptions.builder().withAllocator(allocator).build();
  ...
  public static void readFileIntoWriter(
      byte[] data, BdecParquetWriter outputWriter, BufferAllocator allocator) {
    try (BdecParquetReader reader = new BdecParquetReader(data, allocator)) {
